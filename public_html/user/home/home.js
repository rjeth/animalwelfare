...
function validYr() {
    if ($('#pet_yr').val() == '') {
        $('#pet_yr').addClass('is-invalid');
        $('#pet_age_message').html('Please enter yr');
        bool = false;
    } else if ($('#pet_yr').val() <= 0) {
        $('#pet_age_message').html('Please a valid age');
        bool = false;
    } else {
        $('#pet_yr').removeClass('is-invalid');
        bool = true;
    }
}
function validMonth() {
    if ($('#pet_month').val() == '') {
        $('#pet_month').addClass('is-invalid');
        $('#pet_age_message').html('Please enter yr');
        bool = false;
    } else if ($('#pet_month').val() <= 0) {
        $('#pet_age_message').html('Please a valid age');
        bool = false;
    } else {
        $('#pet_month').removeClass('is-invalid');
        bool = true;
    }
}
...
$('#pet_yr').blur(function () {
    validYr();
});
$('#pet_month').blur(function () {
    validMonth();
});
...
$('button[name="edit"]').click(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#pet_yr').val(data.pet_yr);
            $('#pet_month').val(data.pet_month);
